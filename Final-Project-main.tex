...
\begin{figure*}[!b]
    \centering
    \input[width=0.9\linewidth]{figure/tikz/overall_converter}
...
The push--pull structure distributes the input voltage across two switches, reducing device voltage stress and improving power handling capability under a \qty{25}{\volt} input. 
Furthermore, the $\Phi_2$ topology allows component values to be calculated using closed-form equations, simplifying the design and ensuring reliable \unit{\mega\hertz}-frequency operation.
...
The disadvantages of the design are the physical size, circuit complexity, and higher cost compared to simpler designs such as Class D or Class E based WPT solutions.
...
After the front-end push--pull $\Phi_2$ converter was constructed, it was first tested under a no-load condition. Two complementary 0--5~V square-wave gate-drive signals with a 180$^\circ$ phase shift were generated using a \verb|Keysight 33510B| waveform generator. 
...
The $|Z11|$ measurement in \cref{fig:awr-Z11} shows that \qty{6.78}{\mega\hertz} is extremely close to our resonant frequency.
The $S21$ measurement in \cref{fig:awr-S21} shows that there is decent coupling around \qty{6.78}{\mega\hertz}, with relatively minimal losses.
...
Using a scope with more channels to analyze more parameters simultaniously would also help troubleshooting, as it's possible that some component limits were being briefly exceeded.
