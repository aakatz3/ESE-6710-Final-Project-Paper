...
\begin{figure*}[H]
    \centering
    %[width=0.9\linewidth]
%    \includestandalone{figure/tikz/overall_converter}
...
Originally, \qty{6}{\mega\hertz}--\qty{7}{\mega\hertz} was tested, but this was reduced after some component failures. However, this may not have been the cause of the failures as another failure did ultimately occur.
Coil separation distance was measured from coil plane to coil plane (center of wire to center of wire).
Once set, the coil separation was not varied, but it was bounded between 1--2\unit{\inch} during the design process, with a target value of \qty{1.5}{\inch}.
...
The values of the implemented compensation capacitors are summarized in \cref{tab:bom}.
...
The PCB is attached to an acrylic mounting plate with M3-\qty{0.5}{\milli\meter}$\times$\qty{6}{\milli\meter} Button Head Cap Screws, \qty{12}{\milli\meter} standoffs, and M3 nuts (acting as spacers).
The acrylic mounting plate is attached to the framing via four M5-\qty{0.8}{\milli\meter}\times\qty{12}{\milli\meter} flanged button-head cap screws, with four Nylon Plastic Washers as spacers, into four self-aligning T-slotted framing nuts with M5 threads.
\begin{figure}
    \centering
...
\begin{table}[!h]
    ...
    \resizebox{0.95\columnwidth}{!}{\input{table/results/sims/femm/coils_v1_fixed}}
...
RF Simulations were conducted in Keysight ADS and Cadence AWR Design Environment using the measured transformer parameters to construct the resonant circuit. These simulations were later used to compare with the experimental results of the resonant tank characterization.
...
\begin{figure}[!h]
    \centering
    \includegraphics[width=0.6\columnwidth]{figure/sch/ads-resonant-tank-sch-crop.pdf}
    \caption{ADS Schematic of resonant tank simulation}
    \label{fig:ads-tank-sch}
\end{figure}
\begin{figure}[]
    ...
    \caption{ADS results of resonant tank simulation}
...
The scope, waveform generator, and spectrum analyzer (where applicable) were all supplied with the same 10MHz reference clock (generated from a \verb|Keysight 33521A|).
After the front-end push--pull $\Phi_2$ converter was constructed, it was first tested under a no-load condition. Two complementary 0--5~V square-wave gate-drive signals with a 180\unit{\deg}$^\circ$ phase shift were generated using a \verb|Keysight 33510B| waveform generator. 
...
\begin{figure}[!htb]
    \centering
    \includegraphics[width=0.4\linewidth]{figure/photo/single_coil}
...
\begin{figure}[!htb]
    \centering
    \includegraphics[width=0.4\linewidth]{figure/photo/coil_pair}
...
Parametric studies were performed to assess the converter performance under variations in $V_{\mathrm{in}}$ ($18$--$28~\mathrm{V}$), $D$, $f_s$, and $R_\mathrm{load}$. The drain-to-source voltage waveforms corresponding to different input-voltage operating conditions relative to the nominal value of $V_{\mathrm{in}} = 25~\mathrm{V}$ are shown in \cref{fig:vinsweepwave}. 
...
\subsubsection{Frequenc}
\cref{x1}
\\
\begin{verbbox}
    Spectrum of $P_{MR}$, with 10X passive scope probe, \qty{1}{\mega\ohm} series resistor, and 20dB LNA. Apparent attenuation of \qty{72.833}{\dB}, figure uncorrected
\end{verbbox}
\begin{figure}[!bp]
%        hello
...
While the reference level of the spectrum analyzer is potentially incorrect, a lack of equipment prevented any calibration of the reference level to confirm accuracy.
These results should mainly be used to see the harmonics present at each point measured in the circuit. The multiresonant point spectrum can be seen in \cref{fig:mrspec}.
...
%\begin{verbbox}
%    Spectrum of $P_{D_{AB}}$, with \texttt{N2791A} differential probe at 10X attenuation, \qty{1}{\mega\ohm} series resistor, and 20dB LNA. Assumed attenuation of \qty{72.833}{\dB}, figure uncorrected
%\end{verbbox}
%\begin{figure}[!ht]
%    \centering
...
\begin{figure}[]
    \centering
    \includegraphics[width=0.7\columnwidth]{figure/results/sims/awr/S11-crop.eps}
...
\end{figure}
\section{Conclusion}
This project presented the design and experimental implementation of a 6.78-MHz wireless power transfer converter based on a push--pull Class-$\Phi_2$ topology.
...
Overall, the system achieves wireless power transfer with an output power of up to 36 W, and a peak efficiency of \qty{91.2}{\percent} at the nominal operating point, demonstrating the effectiveness of the proposed design and implementation.
...
\begin{appendices}
    ...
    \end{verbbox}
    \lstinputlisting[language=spice,label=lst:maininc,caption={{\verbBox} LTspice Netlist for converter simulation}] {listing/pushpullphi2_new.sp}
...
%    \FloatBarrier
    ...
    \begin{figure}[H]
        \centering
        \subfloat[]{\includegraphics[height=144pt]{figure/photo/board/RX.png}\label{fig:board:RX}}
        \hfil
    ...
    \begin{table}[H]
        \centering
        \input{table/bom/BOM-CoilInterface.tex}
...
%    \FloatBarrier
    ...
    \begin{figure}[H]
        \centering
        \subfloat[]{\includegraphics[height=144pt]{figure/photo/board/rectifier.png}}
        \hfil
    ...
    \begin{table}[H]
        \centering
        \input{table/bom/BOM-Rectifier.tex}
...
%    \FloatBarrier
    ...
    \begin{table}[H]
        \centering
        \input{table/bom/BOM-SMAtoWago.tex}
